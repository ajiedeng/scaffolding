...
const _logWithTime = (log,level='error')=>console[level](log+',at (milliseconds) '+(new Date().getTime()+'').substr(6));
...
const trimAndCompare = (newState,firstTime)=>{
    ...
    let changedStatus;
    if(newState.status){
        Object.keys(newState.status).forEach(
            (p) => {
                const status = newState.status;

                if(/*filter(p) &&*/ status[p]!=null && status[p]!== currentState.status[p]){
...
const getStatus = function () {
    return sdk.__getStatus__().then((data)=>{
        if(!controlling){
            trimAndCompare(data);
        }
    });
...
const control = (function () {
    ...
    let uuid;
    const retryControl = async function (cmd,{retryCount=0,errorCode=null,timeout = 0} = {}) {
        ...
        return (async function (execId) {
            ...
            for(let i = 0; i < retryCount+1 ; i++) {
                try {
                    ...
                    error = err;
                    if(i < retryCount &&
                        (Array.isArray(errorCode) ? (errorCode.indexOf(err.code) > -1 || errorCode.indexOf(err.code + '') > -1) :  errorCode+'' === err.code+'')) {
                        console.error(`控制出错，${timeout}秒后重试，错误信息：${err.message}`);
    ...
    return async function (cmd,opt) {
        ...
        const {loading,execDelayTimeout,ignoreResponse,retry,updateStrategy,onFail,onSuccess,restartLoopTimeout } = {...defaultOpts,...opt};
...
export { ready,getState,startLoop,stopLoop,control,updateDefaultControlOpts}
